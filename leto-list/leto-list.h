...
#define LETO_T_IFUNC_DATA(name) void (*name)(common)

/// Structure of a singly linked list element
/**
 * This structure implements an element of a singly linked list. These lists
 * can be used not only inside the library, but also outside it. Be careful
 * when working with this structure, remember that it stores a general type
 * pointer to your data.
 */

struct __leto_list_list_t__ {
	struct __leto_list_list_t__ *next; ///< Pointer to the next element
	common data; ///< Pointer to the data stored by the element
};
...
/**
 * List (abbreviation for __leto_list_list_t__) is intended for use outside
 * the library, can be replaced by leto_t_list.
 */
